...
def plot_drive_conditions_stacked(times, states, target_speeds, wind_longs, wind_lats, road_grades, target_headings, psis):
    ...
    # 1. Speed plot
    # ---------------------------
    ax2.plot(times, speed, label="Actual Speed [m/s]", linewidth=2)
    ...
    ax1.set_title("Succesful Vehicle Performance Under Disturbances")
...
def main():
    ...
    for _ in range(N2):
        ...
        avg_speed_error += speed_err / N2
        avg_heading_error += heading_err / N2
    ...
    print("\n")
    print_compact(car)
    print(f"Dynamic Model average error = {avg_speed_error:.5f}, {avg_heading_error:.5f}\n")
    plt.show()

    return avg_heading_error

if __name__ == "__main__":
    avg_avg_heading_err = 0
    for n in range (1):
        print(n)
        avg_avg_heading_err += main() / 1
    
    print(avg_avg_heading_err)
